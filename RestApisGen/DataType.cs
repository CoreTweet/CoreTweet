...
namespace RestApisGen
{
    ...
    {
        private static readonly string[] valueTypes = { "int", "long", "byte", "double", "bool", "UploadMediaType", "TweetMode", "Bucket" };
        ...
        public Method PE
        {
            get
            {
                ...
                else if (this.ReservedNames != null)
                {
                    switch (this.Type)
                    {
                        case ApiType.Normal:
                ...
                else if (l.StartsWith("endpoint"))
                {
                    ...
                    switch (now.Uri?.Count(c => c == '{') ?? 0)
                    {
                        ...
                        case 1:
                            ...
                            break;
                        default:
                            var startIndex = now.Uri.IndexOf('{') + 1;
                            now.ReservedNames = now.Uri.Substring(startIndex, now.Uri.LastIndexOf('}') - startIndex).Split(new [] { "}/{" }, StringSplitOptions.None);
                            break;
                    }
                    mode = Mode.endpoint;
                }
